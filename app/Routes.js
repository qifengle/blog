...
const getGossipPage = (location, callback) => {
	...
	require.ensure([], function (require) {
		...
		document.title = "碎言碎语 - Lipper";
		callback(null, Gossip);
	}, 'gossip');
}

const getMapPage = (location, callback) => {

	require.ensure([], function (require) {
		const { Map, reducer, initialState, stateKey } = require('./containers/Map');
...
const Routes = () => (
	<Router history={history}>
		<Route path="/" breadcrumbName="首页" component={App}>
			...
			<Route name="map" breadcrumbName="图表化" path="map" getComponent={getMapPage} />
		</Route>
	</Router>
)
