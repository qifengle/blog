  ...
  (function () {
    ...
    PIE.Util = {
      ...
      /**
         ...
         */
      withImageSize(src, func, ctx) {
        let size = imageSizes[src],
          img,
          queue;
        if (size) {
 ...
 */
  PIE.GradientUtil = {
    ...
    getGradientMetrics(el, width, height, gradientInfo) {
      let angle = gradientInfo.angle,
      ...
      endPoint = PIE.GradientUtil.perpendicularIntersect(
     ...
     * @return [ x, y ]
     */
    perpendicularIntersect(x1, y1, angle, x2, y2) {
      ...
      // the same x/y values.
      let a1 = Math.tan((angle - 90) * Math.PI / 180),
        c1 = a1 * x1 - y1,
        a2 = -1 / a1,
        c2 = a2 * x2 - y2,
        d = a2 - a1,
 ...
 */
  ...
  PIE.OnUnload.attachManagedEvent(win, 'onresize', () => { PIE.OnResize.fire(); });
  /**
 ...
 */
  ...
  PIE.OnUnload.attachManagedEvent(doc, 'onmouseup', () => { PIE.OnMouseup.fire(); });
  /**
 ...
 */
  PIE.Length = (function () {
    let lengthCalcEl = doc.createElement('length-calc'),
    ...
    Length.prototype = {
      ...
      /**
         ...
         */
      getEmPixels(el) {
        let fs = el.currentStyle.fontSize,
          px,
          parent,
          me;
        ...
        if (fs.indexOf('px') > 0) {
          ...
          return PIE.getLength(fs).pixels(parent, () => me.getEmPixels(parent));
        }
 ...
 */
  PIE.BgPosition = (function () {
    let length_fifty = PIE.getLength('50%'),
 ...
 */
  PIE.BgSize = (function () {
    ...
    BgSize.prototype = {
      ...
      pixels(el, areaW, areaH, imgW, imgH) {
        let me = this,
          w = me.w,
          h = me.h,
    ...
    Tokenizer.prototype = {
      ...
      next(forget) {
        ...
        function newToken(type, value) {
          const tok = new Tokenizer.Token(type, value);
          if (!forget) {
            me.tokens.push(tok);
            me.tokenIndex++;
          }
 ...
 */
  ...
  (function () {
    function cacheWhenLocked(fn) {
      const uid = PIE.Util.getUID(fn);
      return function () {
        if (this._locked) {
          const cache = this._lockedValues || (this._lockedValues = {});
 ...
 */
  PIE.BackgroundStyleInfo = PIE.StyleInfoBase.newStyleInfo({
    ...
    parseCss(css) {
          ...
          if (!image.imgType && tokType & tok_type.FUNCTION && tokVal === 'linear-gradient') {
            ...
            while (token = tokenizer.next()) {
              ...
              // "to <side-or-corner>" - can only appear in first spot
                ...
                gradientTo = tokenizer.until(t => !(t && t.tokenType & tok_type.IDENT && (t.tokenValue in tbIdents || t.tokenValue in lrIdents)));
            ...
            tokenizer.prev();
            image.bgPosition = new PIE.BgPosition(tokenizer.until(t => !isBgPosToken(t), false));
      ...
      // Otherwise, use the standard background properties; let IE give us the values rather than parsing them
        ...
        this.withActualBg(PIE.ieDocMode < 9 ?
          function () {
            let propNames = this.propertyNames,
    ...
    getCss: PIE.StyleInfoBase.cacheWhenLocked(function () {
      return this.getCss3() ||
               this.withActualBg(function () {
                 let cs = this.targetElement.currentStyle,
                   propNames = this.propertyNames;
     ...
     */
    getBgAreaSize(bgOrigin, boundsInfo, borderInfo, paddingInfo) {
      let el = this.targetElement,
 ...
 */
  PIE.BorderStyleInfo = PIE.StyleInfoBase.newStyleInfo({
     ...
     */
    withActualBorder(fn) {
      let rs = this.targetElement.runtimeStyle,
 ...
 */
  (function () {
    ...
    let zero = PIE.getLength('0'),
      zeros = {
        tl: zero, tr: zero, br: zero, bl: zero,
      };
    ...
    parseCss(css) {
      ...
      if (css) {
        ...
        function collectSlicesEtc() {
          slices = tokenizer.until(tok => !(tok.tokenType & (NUMBER | PERCENT)));
          ...
          if (isSlash(tokenizer.next())) {
            slashCount++;
            widths = tokenizer.until(token => !token.isLengthOrPercent() && !((token.tokenType & IDENT) && token.tokenValue === 'auto'));

            if (isSlash(tokenizer.next())) {
              slashCount++;
              outsets = tokenizer.until(token => !token.isLength());
            }
          } else {
            tokenizer.prev();
          }
        ...
        p.slice = distributeSides(slices, tok => PIE.getLength((tok.tokenType & NUMBER) ? `${tok.tokenValue}px` : tok.tokenValue));

        if (widths && widths[0]) {
          p.widths = distributeSides(widths, tok => (tok.isLengthOrPercent() ? PIE.getLength(tok.tokenValue) : tok.tokenValue));
        }

        if (outsets && outsets[0]) {
          p.outset = distributeSides(outsets, tok => (tok.isLength() ? PIE.getLength(tok.tokenValue) : tok.tokenValue));
        }
      }
        ...
        function parseItem() {
          ...
          while (token = tokenizer.next()) {
            ...
            if (type & Type.OPERATOR && value === ',') {
              ...
              tokenizer.prev();
              lengths = tokenizer.until(token => !token.isLength());
 ...
 */
  PIE.PaddingStyleInfo = PIE.StyleInfoBase.newStyleInfo({
    ...
    parseCss(css) {
      let tokenizer = new PIE.Tokenizer(css),
        arr = [],
        token;
 ...
 */
  PIE.VmlShape = (function () {
    ...
    VmlShape.prototype = {
      ...
      setSize(w, h) {
        ...
        this.setAttrs('coordsize', `${w * 2},${h * 2}`);
      },

      getStyleCssText() {
        let styles = this[`${attrsPrefix}style`] || {},
          cssText = [],
          p;
      ...
      getMarkup() {
        ...
        function pushElement(name) {
          const attrs = me[attrsPrefix + name];
          if (attrs) {
  ...
  PIE.merge(PIE.RendererBase, {
     ...
     * @return {PIE.VmlShape}
     */
    getShape(name, zIndex) {
      let shapes = this._shapes || (this._shapes = {}),
        shape = shapes[name];
      if (!shape) {
     ...
     */
    getBoxPath(shrinkT, shrinkR, shrinkB, shrinkL, mult, radii) {
      const coords = this.getBoxPathCoords(shrinkT, shrinkR, shrinkB, shrinkL, mult, radii);
     ...
     */
    getBoxPathCoords(shrinkT, shrinkR, shrinkB, shrinkL, mult, radii) {
      let bounds = this.boundsInfo.getBounds(),
        w = bounds.w * mult,
        h = bounds.h * mult,
        M = Math,
        floor = M.floor,
    ...
    hideBorder() {
      ...
      if ((isIE6 && (tag in PIE.childlessElements || tag === 'FIELDSET')) ||
        ...
        if (el.childNodes.length !== 1 || el.firstChild.tagName !== 'ie6-mask') {
          let cont = doc.createElement('ie6-mask'),
            s = cont.style,
            child;
 ...
 */
  PIE.RootRenderer = PIE.RendererBase.newRenderer({
    ...
    isActive() {
      const children = this.childRenderers;
    ...
    getBoxCssText() {
      let el = this.getPositioningElement(),
        par = el,
        docEl,
        parRect,
     ...
     */
    removeShape(shape) {
      let shapes = this._renderedShapes,
        i;
      if (shapes) {
 ...
 */
  PIE.BackgroundRenderer = PIE.RendererBase.newRenderer({
     ...
     */
    draw() {
      const bounds = this.boundsInfo.getBounds();
     ...
     */
    drawBgColor() {
      let props = this.styleInfos.backgroundInfo.getProps(),
        bounds = this.boundsInfo.getBounds(),
        el = this.targetElement,
      ...
      if (color && color.alpha() > 0) {
        ...
        shape.setAttrs('path', this.getBgClipPath(bounds, props.colorClip));
    ...
    drawBgImages() {
      ...
      if (images) {
        ...
        while (i--) {
          img = images[i];
          shape = this.getShape(`bgImage${i}`, this.shapeZIndex + (0.5 - i / 1000));

          shape.setAttrs('path', this.getBgClipPath(bounds, img.bgClip));
          shape.setSize(w, h);
     ...
     * @param {number} index
     */
    positionBgImage(shape, src, index) {
      PIE.Util.withImageSize(src, function (imgSize) {
        ...
        if (elW && elH) {
          let styleInfos = me.styleInfos,
            ...
            adjustedImgSize = (bg.bgSize || PIE.BgSize.DEFAULT).pixels(me.targetElement, bgAreaSize.w, bgAreaSize.h, imgSize.w, imgSize.h),
    ...
    addLinearGradient(shape, info) {
      ...
      for (i = 0; i < stopCount; i++) {
        vmlColors.push(`${stopPx[i] / lineLength * 100}% ${stops[i].color.colorValue(el)}`);
      }
     ...
     */
    hideBackground() {
      const rs = this.targetElement.runtimeStyle;
 ...
 */
  PIE.BorderRenderer = PIE.RendererBase.newRenderer({
    ...
    draw() {
      ...
      if (props) {
        ...
        for (i = j = 0, len = segmentsInfo.length; i < len; i += 2) {
          ...
          shape.setSize(bounds.w, bounds.h);
          shape.setAttrs('path', segmentsInfo[i]);
          shape.setFillAttrs('color', segmentsInfo[i + 1]);
        }
     ...
     */
    dashify(path, startCoord, endCoord, sideWidth, shift, isVertical, style) {
      let dashLength = sideWidth * (style === 'dashed' ? 3 : 1),
      ...
      if (borderInfo.isActive()) {
        ...
        if (borderProps.stylesSame && borderProps.colorsSame && !(styles.t in me.colorManipStyles)) {
          if (colors.t.alpha() > 0) {
            ...
            style = styles.t;
            if (style === 'double') {
              path.push(me.getBoxPath(wT / 3, wR / 3, wB / 3, wL / 3, mult) +
      ...
      PIE.Util.withImageSize(props.src, function (imgSize) {
        ...
        function setSizeAndPos(rect, rectX, rectY, rectW, rectH, sliceX, sliceY, sliceW, sliceH) {
          ...
          if (!imgW || !imgH || !rectW || !rectH || !sliceW || !sliceH) {
            ...
            rectW = max(rectW, 0);
            rectH = max(rectH, 0);
            rect.setAttrs('path', `m0,0l${rectW * 2},0l${rectW * 2},${rectH * 2}l0,${rectH * 2}x`);
            rect.setFillAttrs(
              'src', src,
              'type', 'tile',
              'position', '0,0',
 ...
 */
  PIE.BoxShadowOutsetRenderer = PIE.RendererBase.newRenderer({
    ...
    isActive() {
      const boxShadowInfo = this.styleInfos.boxShadowInfo;
    ...
    draw() {
      ...
      while (i--) {
        ...
        shape = me.getShape(`shadow${i}`, me.shapeZIndex + (0.5 - i / 1000));
 ...
 */
  PIE.ImgRenderer = PIE.RendererBase.newRenderer({
    ...
    draw() {
      ...
      shape.setAttrs('path', this.getBoxPath(
  ...
  PIE.Element = (function () {
    ...
    function Element(el) {
      ...
      function lockAll() {
        boundsInfo.lock();
        for (let i = styleInfosArr.length; i--;) {
      ...
      function initAncestorEventListeners() {
        let watch = el.currentStyle.getAttribute(`${PIE.CSS_PREFIX}watch-ancestors`),
          i,
          a;
        if (watch) {
