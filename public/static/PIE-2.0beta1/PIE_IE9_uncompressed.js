 ...
 */
  PIE.IE9BackgroundRenderer = PIE.RendererBase.newRenderer({
        ...
        if (images) {
          while (img = images[i++]) {
            if (img.imgType === 'linear-gradient') {
              ...
              bgSize = (img.bgSize || PIE.BgSize.DEFAULT).pixels(me.targetElement, bgAreaSize.w, bgAreaSize.h, bgAreaSize.w, bgAreaSize.h);
              bg.push(`url(${me.getGradientImgData(img, bgSize.w, bgSize.h)}) ${
    ...
    bgPositionToString(bgPosition) {
      return bgPosition ? bgPosition.tokens.map(token => token.tokenValue).join(' ') : '0 0';
    },

    getGradientImgData(info, bgWidth, bgHeight) {
      let me = this,
        el = me.targetElement,
 ...
 */
  PIE.IE9BorderImageRenderer = PIE.RendererBase.newRenderer({
    ...
    draw() {
      ...
      PIE.Util.withImageSize(props.src, (imgSize) => {
        let elW = bounds.w,
          elH = bounds.h,
          imgW = imgSize.w,
          imgH = imgSize.h,
        ...
        function addImage(x, y, w, h, cropX, cropY, cropW, cropH, tileW, tileH) {
          patterns.push(`<pattern patternUnits="userSpaceOnUse" id="pattern${i}" ` +
                        ...
                        '</svg>' +
                    '</pattern>');
          rects.push(`<rect x="${x}" y="${y}" width="${w}" height="${h}" fill="url(#pattern${i})" />`);
          i++;
        }
        ...
        if (props.fill) { // center fill
          addImage(
            widthL, widthT, centerW, middleH, sliceL, sliceT, imgCenterW, imgMiddleH,
